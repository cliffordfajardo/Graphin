...
// ❗NOTE: this is a near identical version of `layout/index.ts`
// the diference main difference is this constructor does not
// take in the Graphin Class component as its constructor argument
// because the Graphin Class Component is now a Function Component.
import G6, { Graph, GraphOptions, IGraph } from '@antv/g6';
...
class LayoutControllerV2 {
  graphinContext: constructorArgs;
  ...
  // 是否每个节点都有位置信息
  hasPosition() {
    const { graphinContext } = this;
    const { data = {} } = graphinContext;
  ...
  init() {
    /** 更新布局参数 */
    this.updateOptions();
    const { options, graphinContext } = this;
    const { data } = graphinContext;
    const { type } = options;
  ...
  /** 重新布局 */
  changeLayout() {
    const { graph, data, isTree, layoutCache } = this.graphinContext;
    if (
      !graph ||
      graph.destroyed ||
      !data ||
  ...
  updateOptions = () => {
    ...
    if (isEmpty(this.graphinContext.data)) {
      this.prevOptions = {};
      return;
    }
  ...
  processForce = () => {
    ...
    const self = this;
    const { options, graphinContext } = self;
    ...
    if (isEmpty(graphinContext.data)) {
      return;
    }
    ...
    if (isForceLayout.current && !isSameLayoutType) {
      ...
      self.presetLayout.init(graphinContext.data);
      self.presetLayout.execute();
      self.presetLayout.data = { ...graphinContext.data };
    }
    ...
    if (isForceLayout.current && isForceLayout.prev && !self.hasPosition()) {
      ...
      if (isEmpty(prevData)) {
        ...
        self.presetLayout.init(graphinContext.data);
        self.presetLayout.execute();
        prevData = graphinContext.data;
      }
      graphinContext.data = Tweak(graphinContext.data, prevData);
    }
  ...
  refreshPosition = () => {
    const { graphinContext } = this;
    const { animate, layoutAnimate } = graphinContext.options;
    const { type } = this.options;
...
export default LayoutControllerV2;
